...
static void defineNative(const char *name, NativeFn function) {
  ...
  insertItem(&vm.globals, AS_STRING(vm.stack[0]), vm.stack[1]);
  pop();
  pop();
}
...
static bool call(ObjClosure *closure, int argCount) {
  if (argCount != closure->function->arity) {
    runtimeError("Expected %d args but got %d.", closure->function->arity,
                 argCount);
    return false;
  }
...
static bool callValue(Value callee, int argCount) {
  ...
  runtimeError("Uncallable type called");
  return false;
}
...
static void concatenate() {
  ...
  ObjString *object = takeString(chars, length);
  push(OBJ_VAL(object));
}
...
#define BINARY_OP(valueType, op)                                               \
  ...
  for (;;) {
    ...
    case OP_SET_GLOBAL: {
      ObjString *name = READ_STRING();
      if (insertItem(&vm.globals, name, peek(0))) {
        deleteItem(&vm.globals, name);
        runtimeError("Undefined variable '%s'.", name->chars);
        return INTERPRET_RUNTIME_ERROR;
      }
      break;
